...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    # Get the fft of x
    X = fft(x)
    # Calculate the periods of the sinusoids in samples
    T1 = fs / f1
    T2 = fs / f2
    # print("T1: " + str(T1) + ", T2: " + str(T2))
    # M is to be the smallest positive int for positve half of DFT that only has two non-zero values
    M = int((T1 * T2) / gcd(int(T1), int(T2)))
    # print("M: " + str(M))
    mX = 20*np.log10(np.abs(X[:M//2+1]))
    # plt.plot(mX)
    # plt.show()
    # print("M: " + str(M) + ", mX length: " + str(len(mX)))
    return mX

x = np.sin(2*np.pi*80*np.arange(0, 100)/10000) + np.sin(2*np.pi*200*np.arange(0, 100)/10000)
minimizeEnergySpreadDFT(x, fs=10000, f1=80, f2=200)