...
def optimalZeropad(x, fs, f):
    ...
    Output:
        ...
        mX (numpy a rray) = The positive half of the DFT spectrum of the N point DFT after zero-padding 
                        x appropriately (zero-padding length to be computed). mX is (N/2)+1 samples long
    """
    ## Your code here
    M = len(x)
    x = np.array(x)
    # length of one period of the sinusoid
    T = fs / f
    # optimal length of zero-padded signal to ensure one of DFT bins coincides with the frequency of the sinusoid
    N = int(ceil(M / T) * T)
    print("M (length in samples of x): " + str(M) + ", T (length of sinusoid): " + str(T) + ", N (desired length of zero-padded signal): " + str(N))
    # pad signal to length N
    x = np.pad(x, pad_width=(0, N - len(x)), mode='constant', constant_values=0)
    # print(f'Length of zero-padded signal: {len(x)}')
    # print(f'x is {x}')
    # get fft of padded x
    X = fft(x)
    # print(f'X is {X}')
    # graph only the positive half of the spectrum, which is (N/2)+1 samples long
    mX = 20 * np.log10(abs(X[:(N//2)+1]))
    # # print(f'length of mX: {len(mX)}')
    # plt.plot(mX)
    # plt.show()
    return mX
# x = np.sin(2*np.pi*250*np.arange(0, 210)/10000)
# optimalZeropad(x, fs=10000, f=250)
