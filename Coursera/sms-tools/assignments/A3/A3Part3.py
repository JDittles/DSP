...
def testRealEven(x):
    ...
    """
    ## We get the length of input
    M = len(x)
    # find our split points for zero phase windowing
    hM1 = int(math.floor((M+1)/2))
    hM2 = int(math.floor(M/2))
    # create our buffer
    dftbuffer = np.zeros(M)
    # fill the back half with the front half of x and the front half with the back half of x
    dftbuffer[:hM1] = x[hM2:]
    dftbuffer[hM1:] = x[:hM2]
    # compute the DFT of the zero phase windowed signal
    X = fft(dftbuffer, M)
    #np.imag gives us the phase at a given point which should be "zero" for a real and even signal
    #np.all checks truth for all of array values
    isRealEven = np.all(np.abs(np.imag(X)) < 1e-6)
    print("isRealEven: " + str(isRealEven))
    return (isRealEven, dftbuffer, X)
print(testRealEven(np.array([2,3,4,3,2])))
