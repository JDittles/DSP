...
def extractMainLobe(window, M):
    ...
    ### Your code here
    ## FFT Size (Need not be a power of 2 in this part)
    N = 8*M
    X = fft(w, N)
    mX = 20*np.log10(np.abs(X)+eps)
    i = 1
    while mX[i] > mX[i+1]:
        i += 1
    j = N - 1
    while mX[j] > mX[j-1]:
        j -= 1
    _mX = np.concatenate((mX[0:i+1], mX[j:]))
    plt.plot(_mX)
    # plt.show()
    plt.title('Magnitude array samples' + str(len(_mX)))
    return _mX