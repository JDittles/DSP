...
def computeSNR(inputFile, window, M, N, H):
    ...
    """
    ## your code here
    convert_to_db = lambda signal: 10 * np.log10(np.sum(signal ** 2) + eps)
    x = UF.wavread(inputFile)[1]
    x_reconstructed = stft.stft(x=x, w=get_window(window, M), N=N, H=H)
    SNR1 = convert_to_db(x) - convert_to_db(x - x_reconstructed)
    SNR2 = convert_to_db(x[M:-M]) - convert_to_db(x[M:-M] - x_reconstructed[M:-M])
    print("SNR1: " + str(SNR1) + ", SNR2: " + str(SNR2))
    return (SNR1, SNR2)