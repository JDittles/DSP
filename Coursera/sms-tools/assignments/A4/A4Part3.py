...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    low_bins = np.where((bin_freqs > 0) & (bin_freqs < 3000))[0]
    high_bins = np.where((bin_freqs > 3000) & (bin_freqs < 10000))[0]
    ...
    plt.pcolormesh(frmTime, bin_freqs[low_bins], np.transpose(mX[low_bins, :]))
    plt.title('Low Bins, Hamming window, M=1001, N=1024, H=256')
    plt.autoscale(tight=True)

    plt.subplot(212)
    # Size of X must be 1 larger than the size of np.diff(pX) (which has the Y axis size
    # reduced by 1) for flat shading
    plt.pcolormesh(frmTime, bin_freqs[high_bins], np.transpose(mX[high_bins, :]))
    plt.title('High Bins')
    plt.autoscale(tight=True)
    sup_title = 'Spectrogram of ' + inputFile + ' ' + window + ' window, M=' + str(M) + ', N=' + str(N) + ', H=' + str(H)
